...
> 状态：政府生鲜与官方设备均已写入公司 V2；I～L 完成，M 因 6 组历史 iMac 重复身份未通过最终验收，待限定修复授权
...
本文表名表示逻辑名称。由于其中 `brand`、`category`、`price_current`、`crawl_run` 和 `crawl_record` 与 V1 同名，当前实现继续使用统一的 `v2_` 物理前缀。公司库的历史重建及 2026-09-10 V1 demo 重采记录保留；阶段 I～L 仅在本地验证，阶段 M 才将官方设备直接采入已有政府价格的 V2，不从 V1 导入。

阶段 M 已于 2026-09-11 完成获准备份、恢复验证、公司增量迁移与五品牌复采，head 为 `b72c910e4f31`。数据库审计无 critical、165 份证据引用检查通过，V1/政府既有行及其他 schema 的 11 类结构元数据未变；但严格重放发现 6 组历史 iMac 重复来源身份，Apple 批次除 3 个 Watch 缺口外还有 iMac 覆盖告警。当前未通过最终验收，待限定合并授权，未擅自改键或删除事实，详见[阶段 M 记录](V2_PHASE_M_ACCEPTANCE_REPORT.md)。

五品牌官方设备采集已直接接入现有 13 张表，与两个政府来源共用唯一 V2 运行时；电商、超市和个人站仍仅保留框架能力。V1 业务实现和运行时 ORM 已删除，但不删除历史物理表。执行边界见[V2 全品类实施计划](V2_GENERAL_CATALOG_DEVELOPMENT_PLAN.md)，设备细节见[原生采集改造计划](V2_DEVICE_NATIVE_COLLECTION_PLAN.md)。

阶段 I 已实现产品级证据 `entity_type=PRODUCT`、可信无金额状态 `price_type=AVAILABILITY_ONLY` 及配套约束；不新增业务表。阶段 J～K 接通五品牌原生解析、标准建档、共用事务和运行保护；阶段 L 接通只读重放、V2 检查/审计和可选设备调度，并退出 V1 业务链。代码和公司库 head 均为 `b72c910e4f31`，阶段 J～L 没有追加迁移。阶段 M 的公司增量迁移仅更换 4 个 V2 校验触发器和 Alembic 版本，23 张业务表既有行与 DDL 未变。

运行时 SQLAlchemy 元数据仅注册 13 张 V2 表；从 Alembic base 升级仍按未修改的历史链创建 10 张 V1 与 13 张 V2，共 23 张业务表。两者并不冲突：历史表可保留，但运行时不依赖它们。自动生成迁移仅反射 `v2_` 范围，避免将已退出 ORM 的 V1 表或其他非 V2 表误判为应删除对象。
...
此分支解决“商品明确下架或缺货但不再展示价格时，上一条在售价格仍留在当前投影”的问题；历史金额继续保留，当前投影可以指向已证实的无价状态，重新在售后再由新可信价格推进。请求失败、解析失败、发现列表缺失、`UNKNOWN` 或政府均价不能使用这个例外。阶段 K 已接入证据判断和缺失复核；仅完整范围批次累积缺失，达到阈值后仍需明确详情证据。阶段 L 的重放与审计也核对无价状态，不能借用旧金额生成新时点报价。
...
阶段 L 的 `catalog replay --record-id` 使用原记录时点和发现上下文，重放全部 SKU 或政府文档行，分别输出当前静态解析与历史校验/已存事实；不联网、不写库，也不把本次执行当作新报价。旧政府记录可从保存的文章 URL、发布日期和商务部品种 ID 确定恢复上下文；缺少必要证据则明确失败，不回读后来变化的 listing 猜测或补抓。
...
设备单产品持久化事务使用 `READ COMMITTED`，避免空匹配范围的间隙锁阻塞不同来源；父 revision 锁、匹配当前读和唯一约束仍保证同一身份的互斥。批次启动在已有来源/地区命名锁下查询同范围旧批次，不再增加冗余范围锁。事务提交/回滚并归还连接后恢复默认隔离级别，政府事务保持原样。此为局部事务策略，不是数据库全局配置或表结构变更；运行要求见[MySQL 兼容说明](MYSQL_57_COMPATIBILITY.md#22-设备并发写入的事务隔离)。
...
以下仅解释历史概念如何对应 V2，不是数据迁移、物理表改名或兼容查询方案。V1 数据不回填 V2，运行时没有 V1 ORM 或写库路径。
...
本节前半为历史操作记录，不是当前部署指令。阶段 I～L 未重复清库或连接公司数据库。
...
阶段 L 保留历史迁移链及 `v2_` 物理表名；`migrations/schema_scope.py` 将后续自动生成的反射范围限定为 `v2_`，不因删除运行时 V1 模型而生成删旧表迁移，也不管理同库其他非 V2 表。阶段 M 已在获准备份和本地恢复验证后，将公司库从 `96524222b3ec` 增量升级至代码 head `b72c910e4f31`；迁移核验未改变既有业务数据、表 DDL 或其他 schema 结构元数据，未运行测试库清理命令。五品牌复采及 V1/政府隔离核验已完成，但严格重放发现 6 组历史 iMac 重复身份，限定数据合并需另行授权；详见[阶段 M 记录](V2_PHASE_M_ACCEPTANCE_REPORT.md)。
...
- 调度任务表和消息队列表：仅保留显式选择设备渠道的可选简单 scheduler，默认不启动、政府来源不加入；数据库不承担分布式队列职责。
...
5. V1 数据不回填 V2；V1 已于 2026-09-10 重新采集设备 demo，V2 保存政府数据，并已于阶段 M 直接从官方来源重采设备；
6. 第一版继续使用 13 张表，不增加地区维表、别名表、调度表或证据文件表。

阶段 M 尚未通过最终验收：数据库审计 healthy 不能覆盖严格重放发现的 6 组历史 iMac 重复身份。限定合并方案须获准后再执行，并保留全部价格时点与证据，不涉及政府、其他品牌或 V1；3 个 Watch 总价缺口继续单列，不承诺全站或全品类覆盖。原始证据保留期限、大规模历史归档阈值和 V1 历史表最终清理时间后置；物理删表须另行确认，不是当前运行前置条件。
