...
> 状态：政府生鲜结构已有公司部署记录；设备阶段 I～L 本地开发完成，阶段 M 真实/公司验收待实施
...
本文表名表示逻辑名称。由于其中 `brand`、`category`、`price_current`、`crawl_run` 和 `crawl_record` 与 V1 同名，当前实现继续使用统一的 `v2_` 物理前缀。公司库的历史重建及 2026-09-10 V1 demo 重采记录保留；截至最近一次公司验收，V2 保存政府价格。阶段 I～L 未连接公司库，不能将本地设备验证结果表述为公司已采集的数据。
...
阶段 I 已实现产品级证据 `entity_type=PRODUCT`、可信无金额状态 `price_type=AVAILABILITY_ONLY` 及配套约束；不新增业务表。阶段 J～K 接通五品牌原生解析、标准建档、共用事务和运行保护；阶段 L 接通只读重放、V2 检查/审计和可选设备调度，并退出 V1 业务链。代码迁移 head 仍为 `b72c910e4f31`，阶段 J～L 没有追加迁移。公司库最近确认的 head 仍为 `96524222b3ec`，设备真实采集与增量部署留到阶段 M 验收。
...
阶段 L 保留历史迁移链及 `v2_` 物理表名；`migrations/schema_scope.py` 将后续自动生成的反射范围限定为 `v2_`，不因删除运行时 V1 模型而生成删旧表迁移，也不管理同库其他非 V2 表。当前代码 head 为 `b72c910e4f31`，公司最近确认版本为 `96524222b3ec`；阶段 M 应先检查/备份，再执行已验证的增量迁移和 V2 设备直接采集，不运行专用测试库的清理命令。
...
阶段 I～L 的本地实现不等于真实来源覆盖验收；阶段 M 尚需完成官方来源 smoke 和公司库增量采集验收。原始证据保留期限、大规模历史归档阈值和 V1 历史表最终清理时间在真实运行产生容量数据后再决定；物理删表须另行确认，不是当前运行前置条件。
