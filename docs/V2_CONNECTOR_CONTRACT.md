...
> 适用阶段：C 及以后
> 当前实现：两个政府生鲜来源与五品牌官方设备统一接入 V2；阶段 I～L 本地开发完成，阶段 M 真实/公司验收待实施
...
阶段 L 已删除 V1 业务流水线、ORM、种子和运行注册表。运行时只依赖 13 张 V2 表；历史 Alembic 链仍保留 10 张 V1 表的定义，完整迁移会创建 23 张业务表，但历史表不再被业务读写，也不是采集前置条件。本轮未连接真实商城或公司库。
