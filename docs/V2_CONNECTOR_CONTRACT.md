...
> 适用阶段：C 及以后
> 当前实现：两个政府生鲜来源与五品牌官方设备统一接入 V2；I～L 完成，M 因 6 组历史 iMac 重复身份未通过最终验收，待限定修复授权
...
阶段 L 已删除 V1 业务流水线、ORM、种子和运行注册表。运行时只依赖 13 张 V2 表；历史 Alembic 链仍保留 10 张 V1 表的定义，完整迁移会创建 23 张业务表，但历史表不再被业务读写，也不是采集前置条件。阶段 M 已完成获准备份、公司增量迁移、五品牌复采和本地回归；数据库审计及证据引用检查通过，但严格重放发现 6 组历史 iMac 重复身份。Apple 批次除 3 个 Watch 缺口外还有 iMac 覆盖告警，不能将数据库 healthy 当作完整验收成功；待限定数据修复授权，见[阶段 M 记录](V2_PHASE_M_ACCEPTANCE_REPORT.md)。
...
Apple 同一个金额对象同时提供 `raw_amount` 和展示 `amount` 时，两者都必须经过条件价门禁且金额一致，当前价与原价均适用；不能只取裸数值而忽略展示值中的“起”“券后”等条件。比较仅限同一对象，不把旁边的分期、换新或比较价格当作该报价的另一种表示。该门禁与真实复采的验证见[阶段 M 记录](V2_PHASE_M_ACCEPTANCE_REPORT.md)。
...
`db check` 只要求 13 张 V2 表；`db audit` 只读检查 V2 指针、身份、匹配、证据关系、状态语义与批次终态，使用 `--check-artifacts` 才读取本地原始文件。不自动修复，不读取或清理 V1 表，不因从未运行的禁用来源误报。可选 `scheduler --channel <设备渠道>` 仅调度显式选择且启用的设备，仍使用同一 V2 流水线，默认不启动，也不调度政府来源。
...
历史实测中，商务部旧站与 Python/OpenSSL 默认配置握手失败，TLS 1.2 的兼容密码套件可以访问。连接器显式选择 `TLS12_COMPAT`，仍校验证书链和主机名；共享 HTTP 抓取器的默认 TLS 策略以及上海来源均不受影响。本地 fixture 验证不等于来源当前可访问，阶段 M 再按门禁执行真实 smoke。
