...
当前开发先阅读：[V2 全品类采集实施计划](docs/V2_GENERAL_CATALOG_DEVELOPMENT_PLAN.md)及[V2 设备原生采集改造计划](docs/V2_DEVICE_NATIVE_COLLECTION_PLAN.md)。阶段 I～L 已完成；M 的备份、迁移、五品牌采集和本地回归已完成，但严格重放发现 6 组 iMac 重复身份，尚未通过最终验收，等待限定数据修复授权。采集已全部停止，无后台任务。V1 历史基线见[设备官方价格采集项目方案](docs/PROJECT_IMPLEMENTATION_PLAN.md)。
...
- V2 阶段 H：商务部白名单扩展为 15 个品种；公司 `device_price` 在完整备份后重建至当时 head `96524222b3ec`，写入商务部 1,354 条批发价和上海 8 条零售均价；重复采集、证据完整性、其他 schema 不变及 MySQL 5.7 约束均验收通过，未启动 V2 scheduler。
...
- V2 阶段 K（已完成）：其余四品牌原生 V2、整产品变价复抓、发现数量/缺失确认、SKU 遗漏降级和同范围过期批次恢复；旧设备采集入口停用，详见[阶段 K 报告](docs/V2_PHASE_K_BUILD_REPORT.md)。
- V2 阶段 L（已完成）：只读原时点重放、V2 数据与可选文件审计、仅需 13 表的数据库检查、显式设备调度；删除 V1 业务运行依赖和旧命令，保留历史迁移与物理表。验证与剩余边界见[阶段 L 报告](docs/V2_PHASE_L_BUILD_REPORT.md)。真实商城及公司验收仍在 M。
- V2 阶段 M（未通过最终验收，待修复授权）：备份恢复、公司增量迁移及五品牌复采完成，head 为 `b72c910e4f31`；461 项单元测试、MySQL 5.7/8.4 各 128 项集成测试、镜像离线 smoke 通过。数据库审计全部 critical 为 0，165 份证据引用检查通过，V1/政府数据及其他 schema 结构元数据未变；但严格重放发现历史 iMac 记录的 6 组重复身份，不能把审计 healthy 当作验收成功。Apple 批次同时保留 3 个 Watch 缺口及 iMac 覆盖告警；未合并或删除数据，详见[阶段 M 验收记录](docs/V2_PHASE_M_ACCEPTANCE_REPORT.md)。

运行时只注册 13 张 V2 表，`db check/audit` 不依赖 V1。历史 Alembic head 仍保留 V1 10 张表与 V2 13 张表；自动生成迁移仅反射 `v2_` 表，不误删旧表或其他业务表。公司库增量部署保留旧设备 demo 和政府价格，不增加删表迁移或表改名。公司实例仍暂用 root，凭据不写入仓库或 `.env`。新入口见[运行手册](docs/OPERATIONS_RUNBOOK.md#25-五品牌原生-v2-入口阶段-jk)。

代码与公司库 head 均为 `b72c910e4f31`；公司库于 2026-09-11 从 `96524222b3ec` 增量升级，仅更换 4 个 V2 校验触发器和 Alembic 版本，23 张业务表既有行与 DDL 未变。迁移不增删业务表、不修改数据库版本或全局环境；含新类型事实时拒绝直接降级，不能靠删除证据实现回退。
...
`db seed-v2-government` 默认只建立 8 条生鲜品类/规则引用和两个禁用的政府来源，不访问网络。`catalog smoke` 不写 MySQL；`catalog crawl` 只有在门禁开启且数据库来源已显式启用时才会写入 V2 表。商务部省略 `--commodity` 时依次运行 15 个独立数据集；每个页面分别保存证据，不合并成虚假大文件。政府来源不注册调度任务。

`catalog crawl/smoke` 默认被 `LIVE_CRAWL_ENABLED=false` 禁用。真实公开页面测试必须由操作者显式开启；默认匿名标识为 `DevicePriceTestBot/1.0`，不要求填写公司或个人联系方式。旧顶层 `adapters/crawl/smoke/replay` 和 `db seed` 已删除；使用 `catalog` 与 V2 种子命令，不保留兼容包装层。
...
五品牌共用如下 V2 入口，可将渠道换为 `HUAWEI_CN_WEB/XIAOMI_CN_WEB/OPPO_CN_WEB/VIVO_CN_WEB`。真实运行须明确授权并打开门禁；阶段 I～L 只做本地 fixture 验证，不代表真实官网覆盖验收：
...
只读验收（将 `123` 替换为目标库的 V2 抓取记录 ID，原始证据目录须可读）：

```bash
make catalog-replay RECORD_ID=123
make db-check
make db-audit
make db-audit CHECK_ARTIFACTS=1
```

重放不联网、不写库，保留原时点并输出共享证据内全部 SKU/政府行；当前静态校验与原历史校验分开展示，不把待复核记录升级为可信新价格。默认审计检查 V2 关联和业务规则；`CHECK_ARTIFACTS=1` 额外读取证据文件并校验哈希。过期信息只报告 warning，不自动改批次或清理数据。

设备可选调度（默认不启动；必须逐个选择已启用的设备来源，不接受政府来源）：

```bash
LIVE_CRAWL_ENABLED=true make scheduler CHANNELS="APPLE_CN_WEB HUAWEI_CN_WEB"
```

该入口复用同一 V2 流水线及既有采集间隔；不会自动选择全部来源，不新增服务或监控平台。
