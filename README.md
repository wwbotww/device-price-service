...
当前开发先阅读：[V2 全品类采集实施计划](docs/V2_GENERAL_CATALOG_DEVELOPMENT_PLAN.md)及[V2 设备原生采集改造计划](docs/V2_DEVICE_NATIVE_COLLECTION_PLAN.md)。阶段 I～L 已完成，M 真实验收待实施；采集、重放、审计与可选调度统一走 V2，尚未在公司库部署本次改造。V1 历史基线见[设备官方价格采集项目方案](docs/PROJECT_IMPLEMENTATION_PLAN.md)。
...
运行时只注册 13 张 V2 表，`db check/audit` 不依赖 V1。历史 Alembic head 仍保留 V1 10 张表与 V2 13 张表；自动生成迁移仅反射 `v2_` 表，不误删旧表或其他业务表。公司库仍保存旧设备 demo 和政府价格，本轮没有部署设备 V2。不增加删表迁移或表改名。公司实例仍暂用 root，凭据不写入仓库或 `.env`。新入口见[运行手册](docs/OPERATIONS_RUNBOOK.md#25-五品牌原生-v2-入口阶段-jk)。
